...
<script>
	...
	function refreshTabTable( module, class, method, tabLocked){
		if (!tabLocked){
			var selected = $("#backend").data(\'selected.tabs\');
			$("#backend").tabs(\'remove\', selected);
			$("#backend").find(\'[href=#tab_\'+class+\']\').click();
		}

		var selected = $("#backend").data(\'selected.tabs\');
        
		var link = \'/admin/ajax/index.php?mod=\'+module+\'&class=\'+class+\'&method=getTabContent\';
		
		$.get(link, function(data){
			$(\'#tab_\'+class).find(\'#grid\').replaceWith(data);
		});
...
//		var val = $("#"+component+"Grid").getGridParam("selrow");
		...
		if (val!=\'\' && val!=\'undefined\'){
			var title = \''; ?>
<?php echo ((is_array($_tmp='Change')) ? $this->_run_mod_handler('lang', true, $_tmp) : smarty_modifier_lang($_tmp)); ?>
<?php echo '\';
        ...
        if (ids.length > 0){
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    if (result){
                        refreshTabTable(module, class, method);
                    }
                }
            });
