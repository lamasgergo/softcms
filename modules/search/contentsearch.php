...
class SearchContent{
  ...
  function SearchContent($mod_name, $search_arr,$page=0){
   ...
   function getInfo(){
    $sql = "";
    $items = array();
    $sql = $this->db->prepare("SELECT Match(Title, Short_Text, Full_Text) AGAINST ('".implode("|",$this->search_arr)."') as rel,ID, Title, Short_Text, Full_Text, CategoryID, LangID, Created FROM ".DB_PREFIX."cnt_item WHERE Full_Text REGEXP '".implode("|",$this->search_arr)."' OR Title REGEXP '".implode("|",$this->search_arr)."' OR Short_Text REGEXP '".implode("|",$this->search_arr)."' ORDER BY rel DESC"); 
  ...
  function Navigation(){
    $count = 0;
    $sql = $this->db->prepare("SELECT COUNT(*) as cnt FROM ".DB_PREFIX."cnt_item WHERE Full_Text REGEXP '".implode("|",$this->search_arr)."' OR Title REGEXP '".implode("|",$this->search_arr)."' OR Short_Text REGEXP '".implode("|",$this->search_arr)."'");
    $res = $this->db->Execute($sql);
    if ($res && $res->RecordCount() > 0) {
