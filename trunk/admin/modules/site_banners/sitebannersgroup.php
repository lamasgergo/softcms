...
class SiteBannersGroup extends TabElement{
	...
	*/
	...
	function SiteBannersGroup($mod_name,$tabID){
		$this->name=__CLASS__;
		$this->sort_table_fields = "false,'S','N'";
	...
	function getValue(){
	    $sql = $this->db->prepare("SELECT ID, Name, Created FROM ".DB_PREFIX."site_banners_group");
	...
	function formData($form,$id=""){
		if (!empty($id)){
			$sql = $this->db->prepare("SELECT * FROM ".DB_PREFIX."site_banners_group WHERE ID='".$id."'");
			$res = $this->db->Execute($sql);
			if ($res && $res->RecordCount() > 0){
				$this->smarty->assign("items_arr",$res->getArray());
			} else $this->smarty->assign("items_arr",array());
	...
	function sitebannersgroup_form($form,$tab_id,$id=""){
	...
	function sitebannersgroup_add($data){
		$sql = $this->db->prepare("INSERT INTO ".DB_PREFIX."site_banners_group (Name,Created) VALUES ('".$data["Name"]."',NOW())");
		$objResponse = new xajaxResponse();			
	...
	function sitebannersgroup_change($data){
		$error = false;
		$sql = $this->db->prepare(
			"UPDATE ".DB_PREFIX."site_banners_group SET Name = '".$data["Name"]."'	WHERE ID='".$data["ID"]."'
		");
		if ($this->db->Execute($sql)){
	...
	function sitebannersgroup_delete($id){
		$sql = $this->db->prepare("DELETE FROM ".DB_PREFIX."site_banners_group WHERE ID='".$id."'");
		if ($this->db->Execute($sql)){
	...
	function sitebannersgroup_refresh($tab_id){
		$objResponse = new xajaxResponse();
		$this->setClassVars();
		$this->setTemplateVars();		
