<?php

/* WebProfilerBundle:Collector:logger.twig */
class __TwigTemplate_ce4f7ad6b0f1d186448b960c06d6b017 extends Twig_Template
{
    protected $parent;

    public function __construct(Twig_Environment $env)
    {
        parent::__construct($env);

        $this->blocks = array(
            'toolbar' => array($this, 'block_toolbar'),
            'menu' => array($this, 'block_menu'),
            'panel' => array($this, 'block_panel'),
        );
    }

    public function getParent(array $context)
    {
        if (null === $this->parent) {
            $this->parent = $this->env->loadTemplate("WebProfilerBundle:Profiler:layout.twig");
        }

        return $this->parent;
    }

    public function display(array $context, array $blocks = array())
    {
        $this->getParent($context)->display($context, array_merge($this->blocks, $blocks));
    }

    // line 3
    public function block_toolbar($context, array $blocks = array())
    {
        echo "<img style=\"margin: 0 5px 0 10px; vertical-align: middle; height: 24px\" width=\"24\" height=\"24\" alt=\"Logs\" src=\"data:image/png;base64,\" />
<span>";
        // line 5
        echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'collector', '5'), "counterrors", array(), "any"), "html");
        echo "</span>
";
    }

    // line 8
    public function block_menu($context, array $blocks = array())
    {
        echo "<span class=\"count\">";
        // line 9
        echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'collector', '9'), "counterrors", array(), "any"), "html");
        echo "</span>
<img style=\"margin: 0 5px 0 0; vertical-align: middle; width: 32px\" width=\"32\" height=\"32\" alt=\"Logs\" src=\"";
        // line -1
        echo twig_raw_filter(        // line 10
$this->env->getExtension("templating")->getContainer()->get("templating.helper.assets")->getUrl("bundles/webprofiler/images/logger.png"));
        echo "\" />
Logs
";
    }

    // line 14
    public function block_panel($context, array $blocks = array())
    {
        echo "    <h2>Logs</h2>

    ";
        // line 17
        if ($this->getAttribute($this->getContext($context, 'collector', '17'), "logs", array(), "any")) {
            echo "        <ul class=\"alt\">
            ";
            // line 19
            $context['_parent'] = (array) $context;
            $context['_seq'] = twig_iterator_to_array($this->getAttribute($this->getContext($context, 'collector', '19'), "logs", array(), "any"));
            foreach ($context['_seq'] as $context['i'] => $context['log']) {
                echo "                <li class=\"";
                // line 20
                echo (twig_test_odd($this->getContext($context, 'i', '20'))) ? ("odd") : ("even");
                if (("ERR" == $this->getAttribute($this->getContext($context, 'log', '20'), "priorityName", array(), "any"))) {
                    echo " error";
                }
                echo "\">
                    ";
                // line 21
                echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'log', '21'), "priorityName", array(), "any"), "html");
                echo "
                    ";
                // line 22
                echo twig_escape_filter($this->env, $this->getAttribute($this->getContext($context, 'log', '22'), "message", array(), "any"), "html");
                echo "
                </li>
            ";
            }
            $_parent = $context['_parent'];
            unset($context['_seq'], $context['_iterated'], $context['i'], $context['log'], $context['_parent'], $context['loop']);
            $context = array_merge($_parent, array_intersect_key($context, $_parent));
            // line 24
            echo "        </ul>
    ";
        } else {
            // line 26
            echo "        <em>No logs available.</em>
    ";
        }
    }

    public function getTemplateName()
    {
        return "WebProfilerBundle:Collector:logger.twig";
    }
}
