...
class HeaderBag {
    ...
    public function get($key, $default = null, $first = true) {
        ...
        if (!array_key_exists($key, $this->headers)) {
            if (null === $default) {
                return $first ? null : array(); } else {
                return $first ? $default : array($default); } }
        if ($first) {
            return count($this->headers[$key]) ? $this->headers[$key][0] : $default; } else {
...
class Request {
    ...
    public function getClientIp($proxy = false) {
        if ($proxy) {
            if ($this->server->has('HTTP_CLIENT_IP')) {
                return $this->server->get('HTTP_CLIENT_IP'); } elseif ($this->server->has('HTTP_X_FORWARDED_FOR')) {
    ...
    public function getHost() {
        if ($host = $this->headers->get('X_FORWARDED_HOST')) {
            ...
            $host = trim($elements[count($elements) - 1]); } else {
    ...
    protected function convertFileInformation(array $files) {
        ...
        foreach ($fixedFiles as $key => $data) {
            if (is_array($data)) {
                ...
                sort($keys);
                if ($keys != $fileKeys) {
                    $fixedFiles[$key] = $this->convertFileInformation($data); } else if ($data['error'] === UPLOAD_ERR_NO_FILE) {
                    $fixedFiles[$key] = null; } else {
