...
class Plot extends TabElement {
	...
	*/
	...
	function getTreeValues($parent_id=0, $ret=array(), $depth=0){
		$depth++;
		$sql = "SELECT * FROM ".DB_PREFIX."cnt_category WHERE ParentID='".$parent_id."' AND LangID='".$this->user->edit_lang_id."' ORDER BY ID";
	...
	function getValue(){
		...
		$sql = "SELECT * FROM ".DB_PREFIX."plots ORDER BY Num ASC";
		$res = $this->db->Execute($sql);
		if ($res && $res->RecordCount() > 0){
    ...
    function formData($form,$id=""){
		...
		if (!empty($id)){
			$sql = $this->db->prepare("SELECT * FROM ".DB_PREFIX."plots WHERE ID='".$id."'");
			$res = $this->db->Execute($sql);
			if ($res && $res->RecordCount() > 0){
    ...
    function plot_form($form,$tab_id, $id=""){
    ...
    function plot_add($data){
		$objResponse = new xajaxResponse();
		$error = false;
		if ($this->checkRequiredFields($data)){
			isset($data['Reserved']) ? $Reserved = $data['Reserved'] : $Reserved = 0;
			isset($data['AllowComments']) ? $AllowComments = $data['AllowComments'] : $AllowComments = 0;
			$sql = $this->db->prepare("INSERT INTO ".DB_PREFIX."plots(Num,Reserved,Description) VALUES ('".$data["Num"]."','".$Reserved."','".$data["Description"]."')");
    ...
    function plot_change($data){
		$error = false;
		$msg = "";
		
		if ($this->checkRequiredFields($data)){
		    isset($data['Reserved']) ? $Reserved = $data['Reserved'] : $Reserved = 0;
			$sql = $this->db->prepare("UPDATE ".DB_PREFIX."plots SET Num='".$data["Num"]."',Reserved='".$Reserved."',Description='".$data["Description"]."' WHERE ID='".$data["ID"]."'");
			if ($this->db->Execute($sql)){
    ...
    function plot_delete($id){
		$sql = $this->db->prepare("DELETE FROM ".DB_PREFIX."plots WHERE ID='".$id."'");
		if ($this->db->Execute($sql)){
    ...
    function plot_publish ($id, $value)
    {
        $objResponse = new xajaxResponse();
            if ($value == "true") {
                $value = "1";
                ...
                $value = "0";
            if ($id != "") {
                $sql = $this->db->prepare("UPDATE " . DB_PREFIX . "plots SET Reserved='" . $value . "' WHERE ID='" . $id . "'");
                $res = $this->db->Execute($sql);
            }
        return $objResponse->getXML();
    }
