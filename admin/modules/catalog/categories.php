	...
	*/
    ...
    function categories_add($data){
		...
		if ($this->checkRequiredFields($data)){
			...
			$sql = $this->db->prepare("INSERT INTO ".CAT_PREFIX."category(UserID,ParentID,LangID,Name,Description,Published,Created, LinkAlias, LinkPath) VALUES ('".$this->user->id."','".$data["ParentID"]."','".$this->user->edit_lang_id."','".$data["Name"]."','".$data["Description"]."','".$published."',NOW(),'".$data["LinkAlias"]."', '".$this->getLinkPath($data['ParentID'])."');");
    ...
    function categories_change($data){
		...
		if ($this->checkRequiredFields($data)){
		    isset($data['Published']) ? $published = $data['Published'] : $published = 0;
			$sql = $this->db->prepare("UPDATE ".CAT_PREFIX."category SET ParentID='".$data["ParentID"]."',Name='".$data["Name"]."',Description='".$data["Description"]."',Published='".$published."', LinkAlias='".$data["LinkAlias"]."', LinkPath='".$this->getLinkPath($data['ParentID'])."' WHERE ID='".$data["ID"]."'");
			if ($this->db->Execute($sql)){
