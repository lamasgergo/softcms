<script language="JavaScript">
  function set_action_{$tab_prefix}(act){ldelim}
	...
	if (act=="copy"){ldelim}
      ...
      {rdelim}
      if (boxcount==1){ldelim}
        {$tab_prefix}_copyform(id);
      {rdelim}
      if(boxcount>1){ldelim}
        alert("{lang checkbox_greater_then_1}"); 
      {rdelim}
      if(boxcount==0){ldelim}
        alert("{lang check_items_for_copy}"); 
      {rdelim}
    {rdelim}
...
<tr>
    <td>
      <img src="{$images}/menu/add.gif" border="0" title="{$prefix|cat:'_add'}" style="cursor:hand;" onclick="set_action_{$tab_prefix}('add');">
    </td>
    <td>
      <img src="{$images}/menu/change.gif" border="0" title="{$prefix|cat:'_change'}"onclick="set_action_{$tab_prefix}('change');" style="cursor:hand;">
    </td>
    <td>
      <img src="{$images}/menu/del.gif" border="0" title="{$prefix|cat:'_delete'}"onclick="set_action_{$tab_prefix}('delete');" style="cursor:hand;">
    </td>
	<td>
      <img src="{$images}/menu/copy.gif" border="0" title="{$prefix|cat:'_copy'}"onclick="set_action_{$tab_prefix}('copy');" style="cursor:hand;">
    </td>
</tr>
</table>