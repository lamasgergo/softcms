...
class CatalogMenu{
	...
	function CatalogMenu($block_vars,$dynamic){
		global $smarty,$db,$lang,$language,$LangID;
		$this->moduleName = 'CatalogMenu';		
		$this->db = $db;
		$this->smarty = $smarty;
		$this->lang = $lang;
	...
	function getTreeList($parent_id=0, $ret=array(), $depth=0){
		$depth++;
		$sql = "SELECT * FROM ".CAT_PREFIX."category WHERE ParentID='".$parent_id."' AND LangID='".$this->LangID."' AND Published='1' ORDER BY ID";
		...
		if ($res && $res->RecordCount() > 0 && $depth <=$this->limitDepth){
			...
			while (!$res->EOF){
				...
				$node['link'] = CATALOG_ROOT_URL.'/'.$res->fields["LinkPath"].'/'.$res->fields["LinkAlias"].'/';
	...
	function getParent($id){
		$ret = 0;
		$sql = $this->db->prepare("SELECT ParentID FROM ".CAT_PREFIX."category WHERE ID='".$id."'");
		$res = $this->db->Execute($sql);
	...
	function appendChilds($items){
		foreach ($items as $id=>$item){
			$sql = $this->db->prepare("SELECT * FROM ".CAT_PREFIX."category WHERE ParentID='".$item['id']."' ORDER BY ID");
			$res = $this->db->Execute($sql);
			if ($res && $res->RecordCount() > 0){
				while (!$res->EOF){
					$node['id'] = $res->fields["ID"];
