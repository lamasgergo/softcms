...
class Categories extends TabElement {
	...
	*/
	...
	function getTreeValues($parent_id=0, $ret=array(), $depth=0){
		$depth++;
		$sql = "SELECT * FROM ".GAL_PREFIX."category WHERE ParentID='".$parent_id."' AND LangID='".$this->user->edit_lang_id."' ORDER BY ID";
    ...
    function formData($form,$id=""){
		...
		if (!empty($id)){
			$sql = $this->db->prepare("SELECT * FROM ".GAL_PREFIX."category WHERE ID='".$id."'");
			$res = $this->db->Execute($sql);
			if ($res && $res->RecordCount() > 0){
				$values = $res->getArray();
    ...
    function categories_add($data){
		...
		if ($this->checkRequiredFields($data)){
			...
			$sql = $this->db->prepare("INSERT INTO ".GAL_PREFIX."category(UserID,ParentID,LangID,Name,Description,Published,Created) VALUES ('".$this->user->id."','".$data["ParentID"]."','".$this->user->edit_lang_id."','".$data["Name"]."','".$data["Description"]."','".$published."',NOW())");
    ...
    function categories_change($data){
		...
		if ($this->checkRequiredFields($data)){
		    isset($data['Published']) ? $published = $data['Published'] : $published = 0;
			$sql = $this->db->prepare("UPDATE ".GAL_PREFIX."category SET ParentID='".$data["ParentID"]."',Name='".$data["Name"]."',Description='".$data["Description"]."',Published='".$published."' WHERE ID='".$data["ID"]."'");
			if ($this->db->Execute($sql)){
    ...
    function categories_delete($id){
		$sql = $this->db->prepare("DELETE FROM ".GAL_PREFIX."category WHERE ID='".$id."'");
		if ($this->db->Execute($sql)){
		    $sql = $this->db->prepare("DELETE FROM ".GAL_PREFIX."category WHERE ParentID='".$id."'");
		    $this->db->Execute($sql);
    ...
    function categories_publish ($id, $value)
    {
        ...
        if (check_rights('publish')) {
            if ($value == "true") {
                ...
                $value = "0";
            if ($id != "") {
                $sql = $this->db->prepare("UPDATE " . GAL_PREFIX . "category SET Published='" . $value . "' WHERE ID='" . $id . "'");
                $res = $this->db->Execute($sql);
            }
        } else
    ...
    function get_category_row_options ($id, $cur_id = 0, $langid = 0, $userid = '', $depth = 0, $row = array())
    {
        ...
        $sql = $this->db->prepare("SELECT * FROM " . GAL_PREFIX . "category WHERE ParentID='" . $id . "' " . $userid_sql . " AND ID<>'" . $cur_id . "' " . $langid_sql . " ORDER BY Name ASC");
        $res = $this->db->Execute($sql);
        if ($res && $res->RecordCount() > 0) {
            while (! $res->EOF) {
                $depth_value = "";
